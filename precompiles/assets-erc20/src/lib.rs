...
pub type BalanceOf<Runtime> = <Runtime as pallet_assets::Config>::Balance;

/// Alias for the Asset Id type for the provided Runtime.
pub type AssetIdOf<Runtime> = <Runtime as pallet_assets::Config>::CurrencyId;
...
pub struct Erc20AssetsPrecompileSet<Runtime>(PhantomData<(Runtime,)>);

impl<Runtime> Erc20AssetsPrecompileSet<Runtime> {
	pub fn new() -> Self {
		Self(PhantomData)
	}
}

impl<Runtime> PrecompileSet for Erc20AssetsPrecompileSet<Runtime>
where
	...
	Runtime: AddressToAssetId<AssetIdOf<Runtime>>,
	<<Runtime as frame_system::Config>::RuntimeCall as Dispatchable>::RuntimeOrigin: OriginTrait,
{
	fn execute(&self, handle: &mut impl PrecompileHandle) -> Option<EvmResult<PrecompileOutput>> {
		...
		if let Some(asset_id) = Runtime::address_to_asset_id(address) {
			...
			if pallet_assets::Pallet::<Runtime>::total_issuance(asset_id) > 0_u32.into() {
				let result = {
	...
	fn is_precompile(&self, address: H160) -> bool {
		if let Some(asset_id) = Runtime::address_to_asset_id(address) {
			...
			!pallet_assets::Pallet::<Runtime>::total_issuance(asset_id).is_zero()
		} else {
			false
		}
	}
	...
	<<Runtime as frame_system::Config>::RuntimeCall as Dispatchable>::RuntimeOrigin: OriginTrait,
{
	fn total_supply(
		...
		// Fetch info.
		let amount: U256 = pallet_assets::Pallet::<Runtime>::total_issuance(asset_id).into();
	...
	fn balance_of(
		...
		let amount: U256 = {
			...
			pallet_assets::Pallet::<Runtime>::balance(asset_id, &owner).into()
		};
	...
	fn approve(
...

		{
			...
			if pallet_assets::Pallet::<Runtime>::allowance(asset_id, &origin, &spender)
			!= 0u32.into()
			{
	...
	fn name(
		...
		Ok(succeed(
			EvmDataWriter::new()
				.write::<Bytes>(
					pallet_assets::Pallet::<Runtime>::asset_info_of(asset_id)
						.unwrap()
						.name
						.as_slice()
						.into(),
	...
	fn decimals(
		...
		Ok(succeed(
			EvmDataWriter::new()
				.write::<u8>(
					pallet_assets::Pallet::<Runtime>::asset_info_of(asset_id).unwrap().decimals,
				)
				.build(),
		))
	}
	...
	fn minimum_balance(
		...
		let min_balance: U256 = pallet_assets::Pallet::<Runtime>::minimum_balance(asset_id).into();
