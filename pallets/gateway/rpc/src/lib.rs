...
impl<C, B> Gateway<C, B> {
	...
	pub fn new(client: Arc<C>) -> Self {
		Gateway { client, _marker: Default::default() }
	}
}
...
pub trait GatewayApi<BlockHash, CurrencyId, AccountId, Balance> {
	...
	#[rpc(name = "riogateway_try_get_current_strategy_unlocked_rewards")]
	fn try_get_current_strategy_unlocked_rewards(&self, at: Option<BlockHash>) -> Result<Balance>;

	#[rpc(name = "riogateway_try_get_unlocked_rewards")]
	fn try_get_unlocked_rewards(&self, at: Option<BlockHash>) -> Result<(Balance, bool)>;

	#[rpc(name = "riogateway_try_calculate_price")]
	fn try_calculate_price(&self, at: Option<BlockHash>) -> Result<Price>;
}
	...
	Balance: Clone + std::fmt::Display + Codec + ToString,
{
	...
	fn try_get_unlocked_rewards(
		...
		let api = self.client.runtime_api();
		let at = BlockId::hash(at.unwrap_or_else(|| self.client.info().best_hash));
		api.try_get_unlocked_rewards(&at).map_err(runtime_error_into_rpc_err)
	}

	fn try_calculate_price(&self, at: Option<<Block as BlockT>::Hash>) -> Result<Price> {
		let api = self.client.runtime_api();
		let at = BlockId::hash(at.unwrap_or_else(|| self.client.info().best_hash));
		api.try_calculate_price(&at).map_err(runtime_error_into_rpc_err)
	}
}
...
pub fn try_hex_or_str(src: &[u8]) -> String {
	let should_as_string =
		src.iter()
			.try_for_each(|c| if b'!' <= *c && *c <= b'~' { Ok(()) } else { Err(()) });
